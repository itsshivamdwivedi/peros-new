...
export async function POST(req: NextRequest) {
  try {
    ...
    tokenData.append("client_version", CLIENT_VERSION);
    ...
    // ✅ Step 2: Create Payment on PhonePe
    const paymentPayload = {
    ...
    const redirectUrl =
      ...
      phonePeRes.data?.data?.redirectUrl ||
      phonePeRes.data?.instrumentResponse?.redirectInfo?.url;

    if (!redirectUrl) {
      console.error("❌ PhonePe returned no redirect URL:", phonePeRes.data);
      return NextResponse.json(
        {
